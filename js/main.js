...
jQuery(document).ready(function($) {
    ...
    function fade() {
        ...
        $('.disappear').each(function() {
            ...
            if ( objectTop > visibilityWindow.min && objectTop < visibilityWindow.max) {
                if ( objectTop > visibilityWindow.min + animation_height && objectTop < visibilityWindow.max - animation_height) {
									// Fully visible
                    $(this).css( {
  ...
  var counter = function() {
	...
	var portraitAnimation = function() {
		...
		for (var i=0; i < cycleThrough.length; i++) {
			uniqTLs[i] = gsap.timeline({repeat: 1, repeatDelay: 1.5, yoyo: true,
				onStart: function(i) {
	...
	var devCoverAnimation = function() {
		...
		var masterTL = gsap.timeline({onComplete: function() {
			gsap.fromTo(".dev-intro", 0.75, {opacity: 0, x: 50}, {opacity: 1, x: 0, delay: -0.125, ease: Power1.easeInOut});
			gsap.fromTo(".intro-button", 0.5, {opacity: 0}, {opacity: 1, delay: 0.5, ease: Power2.easeInOut});
		}});
		var uniqTLs = [];
	...
	var homeCoverAnimation = function() {
		var masterTL = gsap.timeline({
			onComplete: function() {
				gsap.fromTo(".portrait-wrapper", 0.6, {opacity: 0, x: 50}, {opacity: 1, x: 0, ease: Power1.easeInOut});
				gsap.fromTo(".intro-button", 1, {opacity: 0}, {opacity: 1, delay: 0, ease: Power2.easeInOut});
	...
	var headerNavAnimation = function() {
		...
		var splitNav = Splitting({target: activeNav, by: "chars", whitespace: true})[0];
		var splitHeader = Splitting({target: header, by: "chars", whitespace: true})[0];
		...
		function moveNavToHeader() {
			...
			var otherNavTL = gsap.timeline({onComplete: function() {
				// Move parent item over splitted childs
				gsap.set(activeNav, {opacity: 0});
				gsap.set(activeNav, {x: header.getBoundingClientRect().left - activeNav.getBoundingClientRect().left});
			}});
			...
			for (var i=0; i < splitNav.chars.length; i++) {
				...
				var tl = gsap.timeline();
				var left = splitHeader.chars[i].getBoundingClientRect().left - splitNav.chars[i].getBoundingClientRect().left + 2;
				gsap.set(splitNav.chars[i], {fontWeight: "normal"});
				// Text animation
				tl.to(splitNav.chars[i], 0.3, {color: "#444649", y: 100, ease: Power2.easeInOut});
				tl.to(splitNav.chars[i], 0.6, {color: "#444649", x: left, delay: -0.15, ease: Power2.easeInOut});
				tl.to(splitNav.chars[i], 0.3, {color: "white", scale: 1.5, y: 0, opacity: 0, fontWeight: "bold", delay: -0.15, ease: Power2.easeInOut});
				activeNavTL.add(tl, i * 0.1);
			}
			//Move prior siblings to new position
			// if (leftOfActive.length > 0) {
				otherNavTL.to(leftOfActive.reverse(), 1, {x: activeNav.offsetWidth + parseFloat(gsap.getProperty(activeNav, "margin-right")), stagger: 0.3, delay: 0.1, ease: Power2.easyInOut});
			// } 
			
			gsap.set(header, {opacity: 1});
			gsap.set(splitHeader.chars, {y: 50, opacity: 0});
			headerTL.to(splitHeader.chars, 0.15, {opacity: 1, y: 0, stagger: 0.15, delay: 0.6, ease: Bounce});
		}
		...
		function resetNav(href) {
			var navTL = gsap.timeline({onComplete: function() {
				gsap.fromTo(".content", 0.8, {opacity: 1}, {opacity: 0, ease: Power2.easeInOut});
			}});
			...
			if (leftOfActive.length > 0) {
				navTL.to(leftOfActive.reverse(), 0.4, {x: 0, stagger: 0.15, ease: Power2.easyInOut});
			}
			// Animate header text
			gsap.set(activeNav, {opacity: 1, x:0 });
			headerTL.to(splitHeader.chars, 0.4, {opacity: 0, stagger: 0.125});
			tl.to(splitNav.chars, 0.4, {opacity: 1, scale: 1, x: 0, fontWeight: 400, stagger: 0.125, ease: Power2.easeInOut});
		}
	}
	headerNavAnimation();
